    ...
    def forward_core(self, x, t, y):
        ...
        for i, block in enumerate(self.blocks):
            ...
            if (i + 1) == self.encoder_depth:
                """
                x: [N, T, D]
                x.reshape(-1, D) -> [N*T, D] as the input of MLP projector:
                ret: [N*T, z_dim] -> [N, T, z_dim]
                """
                features = [projector(x.reshape(-1, D)).reshape(N, T, -1) for projector in self.projectors]
